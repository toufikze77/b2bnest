...
const ShareTaskDialog: React.FC<ShareTaskDialogProps> = ({ isOpen, onOpenChange, task }) => {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      setLoading(true);
      const shareUrl = `${window.location.origin}/project-management?task=${task.id}`;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Share Task{task ? `: ${task.title}` : ''}</DialogTitle>
          <DialogDescription className="flex items-center gap-2 text-xs">
            <Shield className="w-3 h-3" /> Share with teammates in-platform or via email.
          </DialogDescription>
        </DialogHeader>
